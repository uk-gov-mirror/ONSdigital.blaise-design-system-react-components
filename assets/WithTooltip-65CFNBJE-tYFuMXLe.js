...
import { C as _extends, a as init_theming, h as lightenColor, l as styled, w as init_chunk_LE2LTDW3 } from "./theming-Cwn73GRE.js";
...
//#region node_modules/storybook/dist/_browser-chunks/WithTooltip-65CFNBJE.js
...
function effect(_ref2) {
	...
	return Object.assign(state.elements.popper.style, initialStyles.popper), state.styles = initialStyles, state.elements.arrow && Object.assign(state.elements.arrow.style, initialStyles.arrow), function() {
		Object.keys(state.elements).forEach(function(name) {
			var element = state.elements[name], attributes = state.attributes[name] || {}, style = Object.keys(state.styles.hasOwnProperty(name) ? state.styles[name] : initialStyles[name]).reduce(function(style2, property) {
...
function getOffsetParent(element) {
	for (var window2 = getWindow(element), offsetParent = getTrueOffsetParent(element); offsetParent && isTableElement(offsetParent) && getComputedStyle(offsetParent).position === "static";) offsetParent = getTrueOffsetParent(offsetParent);
...
function arrow(_ref) {
	var _state$modifiersData$, state = _ref.state, name = _ref.name, options = _ref.options, arrowElement = state.elements.arrow, popperOffsets2 = state.modifiersData.popperOffsets, basePlacement = getBasePlacement(state.placement), axis = getMainAxisFromPlacement(basePlacement), len = [left, right].indexOf(basePlacement) >= 0 ? "height" : "width";
...
function effect2(_ref2) {
	var state = _ref2.state, _options$element = _ref2.options.element, arrowElement = _options$element === void 0 ? "[data-popper-arrow]" : _options$element;
...
function mapToStyles(_ref2) {
	...
	var commonStyles = Object.assign({ position }, adaptive && unsetSides), _ref4 = roundOffsets === !0 ? roundOffsetsByDPR({
		x,
		y
	}, getWindow(popper2)) : {
		x,
...
function getScrollParent(node) {
	return [
		...
		"body",
		"#document"
	].indexOf(getNodeName(node)) >= 0 ? node.ownerDocument.body : isHTMLElement(node) && isScrollParent(node) ? node : getScrollParent(getParentNode(node));
}
...
function getClippingParents(element) {
	var clippingParents2 = listScrollParents(getParentNode(element)), clipperElement = ["absolute", "fixed"].indexOf(getComputedStyle(element).position) >= 0 && isHTMLElement(element) ? getOffsetParent(element) : element;
...
function flip(_ref) {
	...
	if (!state.modifiersData[name]._skip) {
		for (var _options$mainAxis = options.mainAxis, checkMainAxis = _options$mainAxis === void 0 ? !0 : _options$mainAxis, _options$altAxis = options.altAxis, checkAltAxis = _options$altAxis === void 0 ? !0 : _options$altAxis, specifiedFallbackPlacements = options.fallbackPlacements, padding = options.padding, boundary = options.boundary, rootBoundary = options.rootBoundary, altBoundary = options.altBoundary, _options$flipVariatio = options.flipVariations, flipVariations = _options$flipVariatio === void 0 ? !0 : _options$flipVariatio, allowedAutoPlacements = options.allowedAutoPlacements, preferredPlacement = state.options.placement, isBasePlacement = getBasePlacement(preferredPlacement) === preferredPlacement, fallbackPlacements = specifiedFallbackPlacements || (isBasePlacement || !flipVariations ? [getOppositePlacement(preferredPlacement)] : getExpandedFallbackPlacements(preferredPlacement)), placements2 = [preferredPlacement].concat(fallbackPlacements).reduce(function(acc, placement2) {
		...
		if (makeFallbackChecks) {
			...
			}, _i = numberOfChecks; _i > 0; _i--) if (_loop(_i) === "break") break;
		}
...
function useGetLatest(val) {
	var ref = import_react$1.useRef(val);
	return ref.current = val, import_react$1.useCallback(function() {
		return ref.current;
	}, []);
}
function useControlledState(_ref) {
	...
	if (initial === void 0 && value === void 0) throw new TypeError("Either \"value\" or \"initial\" variable must be set. Now both are undefined");
	var _React$useState = import_react$1.useState(initial), state = _React$useState[0], setState = _React$useState[1], getLatest = useGetLatest(state), set = import_react$1.useCallback(function(updater) {
...
function usePopperTooltip(config, popperOptions) {
	...
	}, config), defaultModifiers3 = import_react$1.useMemo(function() {
		return [{
			name: "offset",
			options: { offset: finalConfig.offset }
		}];
	}, Array.isArray(finalConfig.offset) ? finalConfig.offset : []), finalPopperOptions = _extends({}, popperOptions, {
		placement: popperOptions.placement || finalConfig.placement,
		modifiers: popperOptions.modifiers || defaultModifiers3
	}), _React$useState = import_react$1.useState(null), triggerRef = _React$useState[0], setTriggerRef = _React$useState[1], _React$useState2 = import_react$1.useState(null), tooltipRef = _React$useState2[0], setTooltipRef = _React$useState2[1], _useControlledState = useControlledState({
		initial: finalConfig.defaultVisible,
		value: finalConfig.visible,
		onChange: finalConfig.onVisibleChange
	}), visible = _useControlledState[0], setVisible = _useControlledState[1], timer = import_react$1.useRef();
	...
	var _usePopper = usePopper(finalConfig.followCursor ? virtualElement : triggerRef, tooltipRef, finalPopperOptions), styles = _usePopper.styles, attributes = _usePopper.attributes, popperProps = _objectWithoutPropertiesLoose(_usePopper, _excluded), update = popperProps.update, getLatest = useGetLatest({
		...
		tooltipRef,
		finalConfig
	}), isTriggeredBy = import_react$1.useCallback(function(trigger) {
		return Array.isArray(finalConfig.trigger) ? finalConfig.trigger.includes(trigger) : finalConfig.trigger === trigger;
	}, Array.isArray(finalConfig.trigger) ? finalConfig.trigger : [finalConfig.trigger]), hideTooltip = import_react$1.useCallback(function() {
		clearTimeout(timer.current), timer.current = window.setTimeout(function() {
			return setVisible(!1);
		}, finalConfig.delayHide);
	}, [finalConfig.delayHide, setVisible]), showTooltip = import_react$1.useCallback(function() {
		clearTimeout(timer.current), timer.current = window.setTimeout(function() {
			return setVisible(!0);
		}, finalConfig.delayShow);
	}, [finalConfig.delayShow, setVisible]), toggleTooltip = import_react$1.useCallback(function() {
	...
	}, [getLatest, hideTooltip]), import_react$1.useEffect(function() {
	...
	var getTooltipProps = function(args) {
		return args === void 0 && (args = {}), _extends({}, args, { style: _extends({}, args.style, styles.popper) }, attributes.popper, { "data-popper-interactive": finalConfig.interactive });
...
var import_react, import_react_dom, import_react$1, import_react$3, import_react$4, import_react$5, import_react_dom$1, require_react_fast_compare, require_warning, import_memoizerific, deprecate, global, fromEntries, useIsomorphicLayoutEffect, top, bottom, right, left, auto, basePlacements, start, end, clippingParents, viewport, popper, reference, variationPlacements, placements, modifierPhases, applyStyles_default, max, min, round, toPaddingObject, arrow_default, unsetSides, computeStyles_default, passive, eventListeners_default, hash, hash2, flip_default, hide_default, offset_default, popperOffsets_default, preventOverflow_default, DEFAULT_OPTIONS, createPopper3, import_react_fast_compare, EMPTY_MODIFIERS, usePopper, noop, _excluded, virtualElement, defaultConfig, document2, match, ArrowSpacing, Arrow, Wrapper, Tooltip, TargetContainer, TargetSvgContainer, WithTooltipPure, WithToolTipState, DeprecatedPure;
//#endregion
__esmMin((() => {
	init_chunk_242VQQM5();
	init_chunk_LE2LTDW3();
	...
	require_react();
	init_theming();
	require_react_fast_compare = __commonJS({ "../../node_modules/react-fast-compare/index.js"(exports, module) {
		...
		function equal(a, b) {
			...
			if (a && b && typeof a == "object" && typeof b == "object") {
				...
				for (i = length; i-- !== 0;) if (!((keys[i] === "_owner" || keys[i] === "__v" || keys[i] === "__o") && a.$$typeof) && !equal(a[keys[i]], b[keys[i]])) return !1;
				return !0;
			}
			return a !== a && b !== b;
		}
	...
	require_warning = __commonJS({ "../../node_modules/warning/warning.js"(exports, module) {
		"use strict";
		var __DEV__ = false, warning2 = function() {};
	...
	import_react$3.createContext();
	import_react$3.createContext();
	fromEntries = function(entries) {
		return entries.reduce(function(acc, _ref) {
			var key = _ref[0];
			return acc[key] = _ref[1], acc;
		}, {});
	}, useIsomorphicLayoutEffect = typeof window < "u" && window.document && window.document.createElement ? import_react$4.useLayoutEffect : import_react$4.useEffect;
	top = "top", bottom = "bottom", right = "right", left = "left", auto = "auto", basePlacements = [
		top,
		bottom,
		right,
		left
	], start = "start", end = "end", clippingParents = "clippingParents", viewport = "viewport", popper = "popper", reference = "reference", variationPlacements = basePlacements.reduce(function(acc, placement) {
	...
	createPopper3 = popperGenerator({ defaultModifiers: [
		eventListeners_default,
		popperOffsets_default,
		computeStyles_default,
		applyStyles_default,
	...
	import_react_fast_compare = __toESM$1(require_react_fast_compare());
	EMPTY_MODIFIERS = [], usePopper = function(referenceElement, popperElement, options) {
		options === void 0 && (options = {});
		var prevOptions = import_react$5.useRef(null), optionsWithDefaults = {
			onFirstUpdate: options.onFirstUpdate,
			placement: options.placement || "bottom",
			strategy: options.strategy || "absolute",
			modifiers: options.modifiers || EMPTY_MODIFIERS
		}, _React$useState = import_react$5.useState({
			styles: {
				popper: {
			...
			attributes: {}
		}), state = _React$useState[0], setState = _React$useState[1], updateStateModifier = import_react$5.useMemo(function() {
			return {
				name: "updateState",
				enabled: !0,
				phase: "write",
				fn: function(_ref) {
					...
					import_react_dom$1.flushSync(function() {
						setState({
		...
		}, []), popperOptions = import_react$5.useMemo(function() {
			var newOptions = {
			...
			updateStateModifier
		]), popperInstanceRef = import_react$5.useRef();
		return useIsomorphicLayoutEffect(function() {
			...
			if (!(referenceElement == null || popperElement == null)) {
				var popperInstance = (options.createPopper || createPopper3)(referenceElement, popperElement, popperOptions);
	...
	__toESM$1(require_warning());
	noop = function() {};
	_excluded = ["styles", "attributes"], virtualElement = { getBoundingClientRect: generateBoundingClientRect() }, defaultConfig = {
	...
	({document: document2} = global), match = (0, import_memoizerific.default)(1e3)((requests, actual, value, fallback = 0) => actual.split("-")[0] === requests ? value : fallback), ArrowSpacing = 8, Arrow = styled.div({
		position: "absolute",
		borderStyle: "solid"
	}, ({ placement }) => {
		let x = 0, y = 0;
		...
		borderTopColor: match("top", placement, theme.color[color] || color || theme.base === "light" ? lightenColor(theme.background.app) : theme.background.app, "transparent"),
		borderBottomColor: match("bottom", placement, theme.color[color] || color || theme.base === "light" ? lightenColor(theme.background.app) : theme.background.app, "transparent"),
		borderLeftColor: match("left", placement, theme.color[color] || color || theme.base === "light" ? lightenColor(theme.background.app) : theme.background.app, "transparent"),
		borderRightColor: match("right", placement, theme.color[color] || color || theme.base === "light" ? lightenColor(theme.background.app) : theme.background.app, "transparent")
	})), Wrapper = styled.div(({ hidden }) => ({
		display: hidden ? "none" : "inline-block",
		zIndex: 2147483647,
		colorScheme: "light dark"
	}), ({ theme, color, hasChrome }) => hasChrome ? {
		background: color && theme.color[color] || color || theme.base === "light" ? lightenColor(theme.background.app) : theme.background.app,
		filter: `
	...
	} : {}), Tooltip = import_react.forwardRef(({ placement = "top", hasChrome = !0, children, arrowProps = {}, tooltipRef, color, withArrows, ...props }, ref) => import_react.createElement(Wrapper, {
		"data-testid": "tooltip",
		hasChrome,
		ref,
		...props,
		color
	}, hasChrome && withArrows && import_react.createElement(Arrow, {
		placement,
		...arrowProps,
		color
	}), children));
...
`, TargetSvgContainer = styled.g`
  cursor: ${(props) => props.trigger === "hover" || props.trigger?.includes("hover") ? "default" : "pointer"};
`, WithTooltipPure = ({ svg = !1, trigger = "click", closeOnOutsideClick = !1, placement = "top", modifiers = [
		{
			name: "preventOverflow",
			options: { padding: 8 }
		},
	...
	], hasChrome = !0, defaultVisible = !1, withArrows, offset: offset2, tooltip, children, closeOnTriggerHidden, mutationObserverOptions, delayHide = trigger === "hover" ? 200 : 0, visible, interactive, delayShow = trigger === "hover" ? 400 : 0, strategy, followCursor, onVisibleChange, portalContainer, ...props }) => {
		let Container = svg ? TargetSvgContainer : TargetContainer, { getArrowProps, getTooltipProps, setTooltipRef, setTriggerRef, visible: isVisible, state } = usePopperTooltip({
			trigger,
			placement,
			defaultVisible,
			delayHide,
			...
			modifiers,
			strategy
		}), portalTarget = (typeof portalContainer == "string" ? document2.querySelector(portalContainer) : portalContainer) || document2.body, tooltipComponent = isVisible ? import_react.createElement(Tooltip, {
			placement: state?.placement,
			ref: setTooltipRef,
			hasChrome,
			...
			withArrows,
			...getTooltipProps()
		}, typeof tooltip == "function" ? tooltip({ onHide: () => onVisibleChange(!1) }) : tooltip) : null;
		return import_react.createElement(import_react.Fragment, null, import_react.createElement(Container, {
			trigger,
			ref: setTriggerRef,
			...props
		}, children), isVisible && import_react_dom.createPortal(tooltipComponent, portalTarget));
	}, WithToolTipState = ({ startOpen = !1, onVisibleChange: onChange, ...rest }) => {
		let [tooltipShown, setTooltipShown] = (0, import_react.useState)(startOpen), onVisibilityChange = (0, import_react.useCallback)((visibility) => {
		...
		}), import_react.createElement(WithTooltipPure, {
			...rest,
			visible: tooltipShown,
			onVisibleChange: onVisibilityChange
		});
	}, DeprecatedPure = (props) => (deprecate("WithTooltipPure is deprecated and will be removed in Storybook 11. Please use WithTooltip instead."), import_react.createElement(WithTooltipPure, {
		"data-deprecated": "WithTooltipPure",
		...props
	}));
}))();
export { WithToolTipState as WithTooltip, DeprecatedPure as WithTooltipPure };
